...
if [ $? -eq 0 ]; then
    ...
    echo ""
    echo "🎯 PRIMARY UNIFIED CLI (recommended):"
    echo "   beautyai --help              # Show all available commands"
    echo "   beautyai run chat            # Start chat interface"
    echo "   beautyai run test            # Run inference tests"
    echo "   beautyai run benchmark       # Run performance benchmarks"
    echo "   beautyai model list          # List available models"
    echo "   beautyai system status       # Show system status"
    echo ""
    echo "📋 LEGACY COMPATIBILITY COMMANDS (still supported):"
    echo "   beautyai-chat                # Legacy chat interface"
    echo "   beautyai-test                # Legacy test script"
    echo "   beautyai-benchmark           # Legacy benchmark tool"
    echo "   beautyai-models              # Legacy model manager"
    echo ""
    echo "For detailed help on any command:"
    echo "   beautyai <command> --help"
    echo ""
    if [[ $install_vllm =~ ^[Yy]$ ]]; then
        echo "vLLM is installed. To use the vLLM engine, use the --engine vllm flag, e.g.:"
        echo "beautyai-chat --engine vllm"
        echo ""
